/*
    ...
    To extend functionality: Add new sources in custom-infrastructure/ directory only or new .hcl file next to it with correct terragrunt configuration.
*/
...
remote_state {
  ...
  config = {
    ...
    key                      = join("/", [local.merged_vars.environment, local.merged_vars.aws_region, local.merged_vars.project_name, "core.tfstate"])
    encrypt                  = true
